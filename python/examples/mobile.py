...
def request(url, params=None, consumer=None, token=None):
        ...
        request = oauth.OAuthRequest(http_method="GET", http_url=url,
...
if __name__ == "__main__":
    ...
    # REQUEST
    resp, content = request(REQUEST_TOKEN_URL, params={}, consumer=consumer,
                            token=None)
    if resp['status'] != '200':
        print content
    ...
    token = oauth.OAuthToken(token_dict['oauth_token'],
                             token_dict['oauth_token_secret'])

    # AUTHORIZATION
    client = oauth.OAuthClient(consumer, token)
    url = "http://api.apontador.com.br/v1/oauth/callback"
    resp, content = request(AUTHORIZATION_URL,
                            params={"oauth_callback": url},
    ...
    resp, content = request(ACCESS_TOKEN_URL,
